...
void StatsRHDialog::loadStatistics()
{
    ...
    // Onglet 3: Parité (maintenant onglet 3 devient onglet 2)
    QWidget *tab3 = tabWidget->widget(2);
    QScrollArea *scroll3 = tab3->findChild<QScrollArea*>();
    if (scroll3) {
        QWidget *container3 = scroll3->widget()->findChild<QWidget*>("graphContainer");
        if (container3) {
            GraphiqueWidget *graph = new GraphiqueWidget(GraphiqueWidget::Camembert,
                                                         effectifSexe,
                                                         "Répartition Hommes / Femmes");
            container3->layout()->addWidget(graph);
        }
    }
}
