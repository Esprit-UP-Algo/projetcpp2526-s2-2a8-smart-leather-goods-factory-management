    ...
    , editingRow(-1)
{
    ...
    // Hide ID column from all tables
...
{
    // Créer un dialogue de tri
    QDialog sortDialog(this);
    sortDialog.setWindowTitle("Trier les Employés");
    sortDialog.setMinimumSize(450, 300);
    sortDialog.setStyleSheet(
        "QDialog { background-color: #FAF5F0; }"
        "QLabel { color: #291C0E; font-family: Arial, sans-serif; font-size: 13px; font-weight: 500; }"
        "QComboBox { "
        ...
        "QRadioButton { color: #291C0E; font-family: Arial, sans-serif; font-size: 12px; spacing: 8px; }"
        "QRadioButton::indicator { width: 16px; height: 16px; }"
        "QRadioButton::indicator { border: 2px solid #BCAAA4; border-radius: 8px; background-color: white; }"
    ...
    QVBoxLayout *mainLayout = new QVBoxLayout(&sortDialog);
    mainLayout->setSpacing(20);
    mainLayout->setContentsMargins(30, 30, 30, 30);
    
    QLabel *titre = new QLabel("[TRI] Options de Tri");
    titre->setStyleSheet("font-size: 18px; font-weight: bold; color: #8D6E63; margin-bottom: 10px;");
    ...
    QComboBox *colonneCombo = new QComboBox(&sortDialog);
    colonneCombo->addItems({"Matricule", "Nom", "Prénom", "CIN", "Téléphone", "Email"});
    
    QWidget *ordreWidget = new QWidget();
    QVBoxLayout *ordreLayout = new QVBoxLayout(ordreWidget);
    ordreLayout->setContentsMargins(0, 0, 0, 0);
    ordreLayout->setSpacing(8);
    
    QRadioButton *radioAsc = new QRadioButton("Croissant (A → Z)");
    QRadioButton *radioDesc = new QRadioButton("Décroissant (Z → A)");
    radioAsc->setChecked(true);
    
    ordreLayout->addWidget(radioAsc);
    ordreLayout->addWidget(radioDesc);
    
    formLayout->addRow("Trier par :", colonneCombo);
    formLayout->addRow("Ordre :", ordreWidget);
    ...
    QPushButton *btnAppliquer = new QPushButton("[OK] Appliquer");
    QPushButton *btnAnnuler = new QPushButton("[X] Annuler");
    btnAnnuler->setStyleSheet("QPushButton { background-color: #95877C; } QPushButton:hover { background-color: #A5978C; }");
    buttonLayout->addWidget(btnAppliquer);
    buttonLayout->addWidget(btnAnnuler);
    mainLayout->addLayout(buttonLayout);
    
    connect(btnAppliquer, &QPushButton::clicked, &sortDialog, [&]() {
        int colIndex = 0;
        QString colName = colonneCombo->currentText();
        if (colName == "Matricule") colIndex = 1;
        else if (colName == "Nom") colIndex = 2;
        else if (colName == "Prénom") colIndex = 3;
        else if (colName == "CIN") colIndex = 4;
        else if (colName == "Téléphone") colIndex = 7;
        else if (colName == "Email") colIndex = 8;
        
        Qt::SortOrder order = radioAsc->isChecked() ? Qt::AscendingOrder : Qt::DescendingOrder;
        ui->employeeTable->sortItems(colIndex, order);
        
        QMessageBox::information(&sortDialog, "Tri Appliqué", 
                                QString("Les employés ont été triés par %1 en ordre %2.")
                                .arg(colName)
                                .arg(radioAsc->isChecked() ? "croissant" : "décroissant"));
        sortDialog.accept();
    });
    connect(btnAnnuler, &QPushButton::clicked, &sortDialog, &QDialog::reject);
...
void MainWindow::on_btnStatistics_clicked()
{
    ...
    int totalEmployees = ui->employeeTable->rowCount();
    
    QMap<QString, int> departmentCount;
    QMap<QString, int> posteCount;
    
    for (int row = 0; row < ui->employeeTable->rowCount(); ++row) {
        QString dept = ui->employeeTable->item(row, 6)->text();
        QString poste = ui->employeeTable->item(row, 7)->text();
        
        departmentCount[dept]++;
        posteCount[poste]++;
    }
    
    // Créer le dialogue de statistiques
    QDialog statsDialog(this);
    statsDialog.setWindowTitle("Statistiques des Employés");
    statsDialog.setMinimumSize(1100, 750);
    statsDialog.setStyleSheet(
        "QDialog { background-color: #FAF5F0; }"
        "QLabel { color: #291C0E; font-family: Arial, sans-serif; font-size: 13px; }"
        "QPushButton { "
        "   background-color: #8D6E63; "
        "   color: white; "
        "   border: none; "
    ...
    QLabel *mainTitle = new QLabel("[STATS] STATISTIQUES DES EMPLOYÉS");
    ...
    statsFrame->setStyleSheet("background-color: #FFF8F0; border: 2px solid #BCAAA4; border-radius: 10px; padding: 20px;");
    QVBoxLayout *statsFrameLayout = new QVBoxLayout(statsFrame);
    
    QLabel *totalLabel = new QLabel(QString("<b style='color: #8D6E63; font-size: 16px;'>Total des Employés:</b> <span style='font-size: 28px; color: #8D6E63; font-weight: bold;'>%1</span>").arg(totalEmployees));
    totalLabel->setAlignment(Qt::AlignCenter);
    statsFrameLayout->addWidget(totalLabel);
    ...
    // ========== GRAPHIQUE CIRCULAIRE - DÉPARTEMENTS ==========
    QVBoxLayout *deptLayout = new QVBoxLayout();
    
    QLabel *deptTitle = new QLabel("[-] Répartition par Département");
    deptTitle->setStyleSheet("font-size: 16px; font-weight: bold; color: #8D6E63; margin-bottom: 10px;");
    deptTitle->setAlignment(Qt::AlignCenter);
    deptLayout->addWidget(deptTitle);
    
    // Créer le graphique circulaire pour les départements
    QPieSeries *deptSeries = new QPieSeries();
    
    QList<QColor> deptColors = {
        QColor("#8D6E63"), QColor("#A0826D"), QColor("#BCAAA4"), 
        QColor("#D7CCC8"), QColor("#6D4C41"), QColor("#5D4037"),
        QColor("#4E342E"), QColor("#3E2723"), QColor("#795548"),
        QColor("#A1887F")
    };
    
    int colorIndex = 0;
    for (auto it = departmentCount.begin(); it != departmentCount.end(); ++it) {
        QPieSlice *slice = deptSeries->append(it.key(), it.value());
        slice->setLabelVisible(true);
        slice->setLabel(QString("%1: %2 (%3%)")
                       .arg(it.key())
                       .arg(it.value())
                       .arg(QString::number((it.value() * 100.0) / totalEmployees, 'f', 1)));
        slice->setColor(deptColors[colorIndex % deptColors.size()]);
        slice->setLabelColor(QColor("#291C0E"));
        slice->setLabelFont(QFont("Arial", 9, QFont::Bold));
        colorIndex++;
    }
    
    QChart *deptChart = new QChart();
    deptChart->addSeries(deptSeries);
    deptChart->setTitle("");
    deptChart->legend()->setVisible(false);
    deptChart->setBackgroundBrush(QBrush(QColor("#FFFFFF")));
    deptChart->setBackgroundRoundness(10);
    
    QChartView *deptChartView = new QChartView(deptChart);
    deptChartView->setRenderHint(QPainter::Antialiasing);
    deptChartView->setMinimumHeight(350);
    deptChartView->setStyleSheet("background-color: white; border: 2px solid #BCAAA4; border-radius: 10px;");
    
    deptLayout->addWidget(deptChartView);
    chartsLayout->addLayout(deptLayout);
    
    // ========== GRAPHIQUE À BARRES - POSTES ==========
    QVBoxLayout *posteLayout = new QVBoxLayout();
    
    QLabel *posteTitle = new QLabel("[-] Répartition par Poste");
    posteTitle->setStyleSheet("font-size: 16px; font-weight: bold; color: #8D6E63; margin-bottom: 10px;");
    posteTitle->setAlignment(Qt::AlignCenter);
    posteLayout->addWidget(posteTitle);
    
    // Créer le graphique à barres pour les postes
    QBarSet *posteSet = new QBarSet("Employés");
    posteSet->setColor(QColor("#8D6E63"));
    posteSet->setBorderColor(QColor("#6D4C41"));
    posteSet->setLabelColor(QColor("#291C0E"));
    
    QStringList posteCategories;
    for (auto it = posteCount.begin(); it != posteCount.end(); ++it) {
        *posteSet << it.value();
        posteCategories << it.key();
    }
    
    QBarSeries *posteSeries = new QBarSeries();
    posteSeries->append(posteSet);
    posteSeries->setLabelsVisible(true);
    posteSeries->setLabelsPosition(QAbstractBarSeries::LabelsOutsideEnd);
    posteSeries->setLabelsPrecision(0);
    
    QChart *posteChart = new QChart();
    posteChart->addSeries(posteSeries);
    posteChart->setTitle("");
    posteChart->setAnimationOptions(QChart::SeriesAnimations);
    posteChart->setBackgroundBrush(QBrush(QColor("#FFFFFF")));
    posteChart->setBackgroundRoundness(10);
    
    QBarCategoryAxis *axisX = new QBarCategoryAxis();
    axisX->append(posteCategories);
    axisX->setLabelsAngle(-45);
    axisX->setLabelsColor(QColor("#291C0E"));
    axisX->setLabelsFont(QFont("Arial", 8, QFont::Bold));
    posteChart->addAxis(axisX, Qt::AlignBottom);
    posteSeries->attachAxis(axisX);
    
    QValueAxis *axisY = new QValueAxis();
    axisY->setRange(0, *std::max_element(posteCount.begin(), posteCount.end()) + 2);
    axisY->setLabelFormat("%d");
    axisY->setLabelsColor(QColor("#291C0E"));
    axisY->setLabelsFont(QFont("Arial", 9));
    posteChart->addAxis(axisY, Qt::AlignLeft);
    posteSeries->attachAxis(axisY);
    
    posteChart->legend()->setVisible(false);
    
    QChartView *posteChartView = new QChartView(posteChart);
    posteChartView->setRenderHint(QPainter::Antialiasing);
    posteChartView->setMinimumHeight(350);
    posteChartView->setStyleSheet("background-color: white; border: 2px solid #BCAAA4; border-radius: 10px;");
    
    posteLayout->addWidget(posteChartView);
    chartsLayout->addLayout(posteLayout);
    
    mainLayout->addLayout(chartsLayout);
    
    // Bouton Fermer
    QPushButton *btnClose = new QPushButton("[OK] Fermer");
...
{
    qDebug() << "populateEmployeeTable called - Loading from Oracle";
    
    ui->employeeTable->setRowCount(0);
    
    EmployeManager empManager;
    QList<Employe> employes = empManager.getAllEmployes();
    
    if (employes.isEmpty()) {
        qDebug() << "⚠️ Aucun employé trouvé dans la base Oracle";
        QMessageBox::warning(nullptr, "Attention", 
            "Aucun employé trouvé dans la base de données.\n"
            "Exécutez le script create_table_employe.sql pour ajouter des données de test.");
        return;
    }
    
    ui->employeeTable->setRowCount(employes.size());
    
    for (int i = 0; i < employes.size(); ++i) {
        const Employe &e = employes[i];
        ui->employeeTable->setItem(i, 0, new QTableWidgetItem(QString::number(e.getId())));
        ui->employeeTable->setItem(i, 1, new QTableWidgetItem(e.getMatricule()));
        ui->employeeTable->setItem(i, 2, new QTableWidgetItem(e.getNom()));
        ui->employeeTable->setItem(i, 3, new QTableWidgetItem(e.getPrenom()));
        ui->employeeTable->setItem(i, 4, new QTableWidgetItem(e.getCin()));
        ui->employeeTable->setItem(i, 5, new QTableWidgetItem(e.getDateNaissance().toString("dd/MM/yyyy")));
        ui->employeeTable->setItem(i, 6, new QTableWidgetItem(e.getDepartement()));
        ui->employeeTable->setItem(i, 7, new QTableWidgetItem(e.getPoste()));
        ui->employeeTable->setItem(i, 8, new QTableWidgetItem(e.getTelephone()));
        ui->employeeTable->setItem(i, 9, new QTableWidgetItem(e.getEmail()));
    }
    
    qDebug() << "✅ Table populated with" << ui->employeeTable->rowCount() << "employees from Oracle";
}
...
void MainWindow::onSuggestionCommande()
{
    ...
    if (suggestions.count('\n') <= 2) {
        suggestions += "[OK] Aucune commande urgente nécessaire.\n";
...
void MainWindow::onOptimisationFIFO()
{
    ...
    fifoAnalysis += "[FIFO] ANALYSE FIFO (First In, First Out)\n\n";
...
void MainWindow::onGestionFournisseurs()
{
    ...
    fournisseurs += "[TEAM] GESTION DES FOURNISSEURS\n\n";
    fournisseurs += "[LIST] Liste des fournisseurs:\n\n";
...
void MainWindow::onModifierProduction()
{
    ...
    QLabel *titre = new QLabel("✎ Modifier la commande");
    titre->setStyleSheet("font-size: 20px; font-weight: bold; color: #8D6E63; margin-bottom: 10px;");
    mainLayout->addWidget(titre);
...
void MainWindow::onExcelProduction()
{
    ...
    QPushButton *btnImprimer = new QPushButton("⇩ Imprimer PDF", &bilanDialog);
...
void MainWindow::onProductionTableContextMenu(const QPoint &pos)
{
    ...
    QAction *actionVoir = contextMenu.addAction("◉ Voir détails");
    QAction *actionModifier = contextMenu.addAction("✎ Modifier");
    contextMenu.addSeparator();
    QAction *actionSupprimer = contextMenu.addAction("× Supprimer");
    contextMenu.addSeparator();
...
void MainWindow::on_btnAideDecision_clicked()
{
    ...
    for (int i = 0; i < articles.size(); ++i) {
        ...
        QString statutText = article.statut == "disponible" ? "✅ Disponible" : 
                            article.statut == "en_production" ? "⚙️ En production" : "❌ Obsolète";
