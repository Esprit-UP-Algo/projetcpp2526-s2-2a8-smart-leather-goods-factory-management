    ...
    , editingRow(-1)
{
    ...
    //           6=Adresse, 7=Téléphone, 8=Email, 9=Poste, 10=Spécialité, 11=Département,
    //           12=DateEmbauche, 13=Photo, 14=ID
    ui->employeeTable->setColumnCount(15);
    ui->employeeTable->setHorizontalHeaderLabels({"Matricule", "Nom", "Prénom", "CIN", 
                                                   "Date Naissance", "Sexe", "Adresse", 
                                                   "Téléphone", "Email", "Poste", "Spécialité", 
                                                   "Département", "Date Embauche", "Photo", "ID"});
    // Cacher: Adresse, Spécialité, DateEmbauche, Photo, ID
    ui->employeeTable->setColumnHidden(6, true);   // Adresse
    ui->employeeTable->setColumnHidden(10, true);  // Spécialité
    ui->employeeTable->setColumnHidden(12, true);  // Date Embauche
    ui->employeeTable->setColumnHidden(13, true);  // Photo
    ui->employeeTable->setColumnHidden(14, true);  // ID
    
    int empWidths[] = {110,100,100,100,120,60,0,110,180,120,0,130,0,0,0};
    for (int i = 0; i < 15; ++i) ui->employeeTable->setColumnWidth(i, empWidths[i]);
...
void MainWindow::on_btnEdit_clicked()
{
    ...
    //           6=Adresse, 7=Téléphone, 8=Email, 9=Poste, 10=Spécialité, 11=Département,
    //           12=DateEmbauche, 13=Photo, 14=ID
    QTableWidgetItem *idItem = ui->employeeTable->item(row, 14);
    ...
    dlg.setEmployeeData(employeeId, cellText(ui->employeeTable,row,0),
                        ...
                        cellText(ui->employeeTable,row,9), cellText(ui->employeeTable,row,10),
                        cellText(ui->employeeTable,row,11), cellText(ui->employeeTable,row,12),
                        cellText(ui->employeeTable,row,13));
...
void MainWindow::on_btnDelete_clicked()
{
    ...
    dlg.setEmployeeData(employeeId, cellText(ui->employeeTable,row,0),
                        ...
                        cellText(ui->employeeTable,row,11),"");
...
void MainWindow::onEmployeeSelected()
{
    ...
    ui->valSpecialite->setText(cellText(ui->employeeTable, row, 10));
    ui->valDepartement->setText(cellText(ui->employeeTable, row, 11));
    ui->valDateEmbauche->setText(cellText(ui->employeeTable, row, 12));
