...
{
    // Connexion série Arduino (conforme atelier : 9600 bauds)
    m_serialArduino = new QSerialPort(this);
    m_serialArduino->setPortName("COM5");       // ⚠️ Adapter selon votre port
    m_serialArduino->setBaudRate(QSerialPort::Baud9600);
    m_serialArduino->setDataBits(QSerialPort::Data8);
    m_serialArduino->setParity(QSerialPort::NoParity);
    m_serialArduino->setStopBits(QSerialPort::OneStop);
    m_serialArduino->setFlowControl(QSerialPort::NoFlowControl);
    
    // ✅ ReadWrite au lieu de ReadOnly — obligatoire sur Windows
    if (m_serialArduino->open(QSerialPort::ReadWrite)) {
        connect(m_serialArduino, &QSerialPort::readyRead,
                this, &MainWindow::onArduinoDataReceived);
        qDebug() << "✅ Arduino Pointage connecté sur COM5";
    } else {
        qDebug() << "⚠️ Arduino non connecté:" << m_serialArduino->errorString();
    }
    ...
    m_timerAbsences->start(60000); // vérifie toutes les minutes
}

// ── Réception données Arduino ─────────────────────────────────────────────
void MainWindow::onArduinoDataReceived()
{
    if (!m_serialArduino) return;
    
    // ✅ Accumuler dans un buffer statique
    static QByteArray buffer;
    buffer += m_serialArduino->readAll();
    ...
    while (buffer.contains('\n')) {
        ...
        qDebug() << "Arduino reçu:" << msg;
        
        if (msg.contains("UID:")) {
            // Extraire tout ce qui vient après le dernier "UID:"
            ...
            if (ok && !m_pointage.estDejaPointe()) {
                ...
                populateEmployeeTable(); // Rafraîchir le tableau
        ...
        // Ignorer ACCES:OK et autres lignes
    }
}
