...
// Styles de boutons réutilisables é définis une seule fois pour éviter la duplication
    ...
    : QDialog(parent), m_mode(mode)
{
    ...
    case AddMode:
        ...
        generateAutoReference(); // Génére CMD-YYYY-NNNN automatiquement
        txtId->setText("Auto");
    ...
    case DeleteMode:
        ...
        lblDeleteWarning->setText("? ATTENTION : Vous étes sur le point de supprimer cette commande.\nCette action est irréversible.");
...
void ProductionDialog::setupUI()
{
    ...
    cmbPriorite->setCurrentIndex(1); // "Normale" par défaut
    ...
    // Bandeau d'avertissement rouge é visible uniquement en mode DeleteMode
...
void ProductionDialog::onSaveClicked()
{
    ...
    NotificationWidget::show(
        m_mode == AddMode ? "Commande créée" : "Commande mise é jour",
        txtReference->text() + " é " + (m_mode == AddMode ? "enregistrée avec succés." : "modifiée avec succés."),
...
{
    if (QMessageBox::question(this,"Confirmation","étes-vous sér de vouloir supprimer cette commande ?",
                              QMessageBox::Yes|QMessageBox::No) == QMessageBox::Yes) {
        NotificationWidget::show(
            "Commande supprimée",
            txtReference->text() + " é supprimée définitivement.",
...
void ProductionDialog::loadEmployes()
{
    ...
    while (query.next()) {
        ...
        // Stocker l'ID comme userData pour le récupérer via getEmployeId()
...
QVariant ProductionViewModel::data(const QModelIndex &index, int role) const
{
    ...
    // Qt::DisplayRole : texte affiché dans chaque cellule
    ...
    // Qt::BackgroundRole : couleur de fond de la cellule Statut selon l'état
    if (role == Qt::BackgroundRole) {
        if (index.column() == COL_STATUT) {
            if (cmd.etatProduction == "Terminé") return QBrush(QColor("#27AE60")); // vert
            if (cmd.etatProduction == "En cours") return QBrush(QColor("#F39C12")); // orange
            if (cmd.etatProduction == "Bloqué") return QBrush(QColor("#E74C3C"));  // rouge
            if (cmd.etatProduction == "Planifié") return QBrush(QColor("#3498DB")); // bleu
        }
    }

    // Qt::ForegroundRole : texte blanc sur les cellules colorées du statut
    ...
    // Qt::TextAlignmentRole : montant aligné é droite, reste é gauche
...
void ProductionViewModel::updateCommande(int row, const ProductionCommande &commande)
{
    if (row >= 0 && row < m_commandes.count()) {
        ...
        // Notifie la vue que toute la ligne a changé (pour rafraéchir les couleurs d'alerte)
...
{
    beginResetModel(); // Signale é la vue que les données vont changer
    m_commandes.clear();

    // Jointure avec EMPLOYES pour afficher le nom complet de l'employé responsable
    QSqlQuery query(Connection::instance()->getDatabase());
    ...
    endResetModel(); // Signale é la vue que les données sont prétes
}
...
ProductionView::ProductionView(QWidget *parent)
    ...
    , m_joursAlerte(3)           // Seuil par défaut : alerte 3 jours avant la date limite
    , m_seuilAvancement(50)      // Seuil par défaut : alerte si avancement < 50%
    , m_intervalleVerification(5) // Vérification toutes les 5 minutes
{
    setWindowTitle("Vue Production Compléte - Planification, Suivi & Livraison");
    setMinimumSize(1200, 650);
    resize(1200, 650);

    setupUI();          // Construit tous les widgets
    setupConnections(); // Connecte les signaux/slots
    setupTimer();       // Démarre le timer de vérification des alertes
    loadData();         // Charge les données depuis la DB
}
...
void ProductionView::setupUI()
{
    ...
    centralWidget->setStyleSheet("background-color: #F8F9FA;");  // Gris trés clair moderne
    ...
    // Modéle source + proxy pour filtrage/tri sans modifier les données originales
    m_model = new ProductionViewModel(this);
    m_proxyModel = new ProductionSortProxy(this);
    m_proxyModel->setSourceModel(m_model);
    m_proxyModel->setFilterCaseSensitivity(Qt::CaseInsensitive);
    m_proxyModel->setFilterKeyColumn(ProductionViewModel::COL_REFERENCE); // Filtre par défaut sur la référence
...
void ProductionView::loadData()
{
    ...
    qDebug() << "Nombre de lignes dans le modéle:" << m_model->rowCount();
    ...
    // Vérifier les alertes dés le chargement (sans attendre le timer)
    verifierAlertes();
...
{
    if (index == 0) {
        // "Tous les états" ? revenir au filtre par référence (barre de recherche)
        m_proxyModel->setFilterKeyColumn(ProductionViewModel::COL_REFERENCE);
        m_proxyModel->setFilterFixedString(m_searchBox->text());
    } else {
        // Filtrer sur la colonne Statut avec la valeur sélectionnée
...
{
    // Le filtre livraison n'est pas géré par le modéle simplifié (champ non chargé depuis DB)
    // On réinitialise simplement le filtre sur la référence si "Tous les statuts" est sélectionné
    if (index == 0) {
        m_proxyModel->setFilterKeyColumn(ProductionViewModel::COL_REFERENCE);
        m_proxyModel->setFilterFixedString(m_searchBox->text());
    }
...
void ProductionView::onDetailsClicked()
{
    ...
    // Convertir l'index proxy ? index source pour accéder aux données réelles
    ...
    // Lambda : crée une section avec titre coloré dans le scroll area
...
void ProductionView::onPlanificationClicked()
{
    ...
    // Convertir l'index proxy ? index source pour modifier les données réelles
    QModelIndex sourceIndex = m_proxyModel->mapToSource(currentIndex);
    ProductionCommande cmd = m_model->getCommande(sourceIndex.row());
    ...
    connect(saveBtn, &QPushButton::clicked, [&]() {
        // Mettre é jour les champs de planification dans l'objet cmd
        cmd.dateDebutPrevue = dateDebut->date();
        cmd.dateFinPrevue = dateFin->date();
        cmd.atelier = atelier->text();
        cmd.ordrePassage = ordre->value();

        // Mettre é jour le modéle en mémoire (pas de persistance DB ici)
        m_model->updateCommande(sourceIndex.row(), cmd);

        NotificationWidget::show(
            "Planification mise é jour",
            cmd.reference + " é dates et atelier enregistrés.",
            NotificationWidget::Success
            );
        dlg.accept();
    });
...
{
    loadData();
    QMessageBox::information(this, "Actualisation", "Données actualisées avec succés!");
}

// ---------------------------------------------------------------------------
// IMPLéMENTATION: Systéme d'alertes de retard
// ---------------------------------------------------------------------------
...
void ProductionView::setupTimer()
{
    ...
    // é chaque expiration du timer, vérifier les alertes de retard
...
// -- Vérification périodique des alertes (appelée par le timer) ---------------
void ProductionView::verifierAlertes()
{
    qDebug() << "verifierAlertes() é" << QTime::currentTime().toString();
    if (!m_model) return;
    ...
    for (int row = 0; row < m_model->rowCount(); ++row) {
        ...
        // Recalculer l'alerte avec les seuils configurés
        ...
        // N'afficher la notification que si l'alerte est nouvelle ET pas encore envoyée
        // Clé unique = idCommande * 10 + niveau_alerte (évite les doublons entre Risque et Retard)
        if (cmd.alerteRetard != ancienneAlerte && cmd.alerteRetard != ProductionCommande::Aucune) {
            ...
            if (!m_alertesNotifiees.contains(cleAlerte)) {
                ...
                m_alertesNotifiees.insert(cleAlerte); // Marquer comme notifiée
                nbAlertes++;
            }
        }

        // Mettre é jour le modéle pour que la couleur de la ligne refléte la nouvelle alerte
...
void ProductionView::recalculerToutesLesAlertes()
{
    ...
    if (!m_model) {
        qDebug() << "? Modéle non initialisé";
        return;
    }

    // Réinitialiser l'historique pour permettre de re-notifier toutes les alertes
    m_alertesNotifiees.clear();

    // Recalculer le niveau d'alerte de chaque commande et mettre é jour le modéle
    for (int row = 0; row < m_model->rowCount(); ++row) {
        ProductionCommande cmd = m_model->getCommande(row);
        cmd.alerteRetard = cmd.calculerAlerteRetard(m_joursAlerte, m_seuilAvancement);
        m_model->updateCommande(row, cmd);
