...
// Styles de boutons réutilisables — définis une seule fois pour éviter la duplication
    ...
    : QDialog(parent), m_mode(mode)
{
    ...
    case AddMode:
        ...
        generateAutoReference(); // Génère CMD-YYYY-NNNN automatiquement
        txtId->setText("Auto");
    ...
    case DeleteMode:
        ...
        lblDeleteWarning->setText("⚠ ATTENTION : Vous êtes sur le point de supprimer cette commande.\nCette action est irréversible.");
...
QVariant ProductionViewModel::data(const QModelIndex &index, int role) const
{
    ...
    // Qt::TextAlignmentRole : montant aligné à droite, reste à gauche
...
void ProductionViewModel::updateCommande(int row, const ProductionCommande &commande)
{
    if (row >= 0 && row < m_commandes.count()) {
        ...
        // Notifie la vue que toute la ligne a changé (pour rafraîchir les couleurs d'alerte)
...
{
    beginResetModel(); // Signale à la vue que les données vont changer
    m_commandes.clear();
...
void ProductionView::setupUI()
{
    ...
    // Modèle source + proxy pour filtrage/tri sans modifier les données originales
...
void ProductionView::loadData()
{
    ...
    // Vérifier les alertes dès le chargement (sans attendre le timer)
    verifierAlertes();
...
{
    // Le filtre livraison n'est pas géré par le modèle simplifié (champ non chargé depuis DB)
...
void ProductionView::onPlanificationClicked()
{
    ...
    connect(saveBtn, &QPushButton::clicked, [&]() {
        // Mettre à jour les champs de planification dans l'objet cmd
...
void ProductionView::setupTimer()
{
    ...
    // À chaque expiration du timer, vérifier les alertes de retard
...
{
    qDebug() << "verifierAlertes() à" << QTime::currentTime().toString();
    if (!m_model) return;
    ...
    for (int row = 0; row < m_model->rowCount(); ++row) {
        ...
        // Mettre à jour le modèle pour que la couleur de la ligne reflète la nouvelle alerte
...
void ProductionView::recalculerToutesLesAlertes()
{
    ...
    // Recalculer le niveau d'alerte de chaque commande et mettre à jour le modèle
    for (int row = 0; row < m_model->rowCount(); ++row) {
        ProductionCommande cmd = m_model->getCommande(row);
        cmd.alerteRetard = cmd.calculerAlerteRetard(m_joursAlerte, m_seuilAvancement);
        m_model->updateCommande(row, cmd);
